...
@section('content')
<div class="card" x-data="formHandler('{{ route('sale-returns.update', $saleReturn) }}', 'PUT')" x-init="init()">
    <div class="card-header">
        <h2 class="card-title">Edit Sale Return</h2>
        ...
        <form @submit.prevent="submit($event)">
                ...
                <div class="form-group">
                    ...
                    <select id="fk_sale_id" x-model="form.fk_sale_id" required>
                        ...
                        @foreach($sales as $sale)
                            <option value="{{ $sale->id }}" {{ $saleReturn->fk_sale_id == $sale->id ? 'selected' : '' }}>{{ $sale->invoice_id }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="fk_stock_id" x-model="form.fk_stock_id" required>
                        ...
                        @foreach($stocks as $stock)
                            <option value="{{ $stock->id }}" {{ $saleReturn->fk_stock_id == $stock->id ? 'selected' : '' }}>{{ $stock->product->name ?? 'Product' }} (Qty: {{ $stock->quantity }})</option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select id="status" x-model="form.status" required>
                        ...
                        <option value="approved" {{ $saleReturn->status == 'approved' ? 'selected' : '' }}>Approved</option>
                        <option value="rejected" {{ $saleReturn->status == 'rejected' ? 'selected' : '' }}>Rejected</option>
                        <option value="completed" {{ $saleReturn->status == 'completed' ? 'selected' : '' }}>Completed</option>
                    </select>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <textarea id="note" x-model="form.note" rows="3">{{ old('note', $saleReturn->note) }}</textarea>
...
<script>
    function formHandler(url, method) {
        return {
            form: {
                fk_sale_id: '{{ old('fk_sale_id', $saleReturn->fk_sale_id) }}',
                fk_stock_id: '{{ old('fk_stock_id', $saleReturn->fk_stock_id) }}',
                quantity: {{ old('quantity', $saleReturn->quantity) }},
                refund_amount: '{{ old('refund_amount', $saleReturn->refund_amount) }}',
                reason: '{{ old('reason', $saleReturn->reason) }}',
                note: @js(old('note', $saleReturn->note)),
                status: '{{ old('status', $saleReturn->status) }}',
            },
            errors: {},
            errorMessage: '',
            successMessage: '',
