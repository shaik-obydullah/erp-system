...
@section('content')
<div class="card" x-data="formHandler('{{ route('brands.update', $brand) }}', 'PUT')" x-init="init()">
    <div class="card-header">
        <h2 class="card-title">Edit Brand: {{ $brand->name }}</h2>
        ...
        return {
            ...
            async submit() {
                ...
                try {
                    ...
                    if (response.ok) {
                        window.location.href = data.redirect || '{{ route("brands.index") }}';
                        return;
                    }
