...
@section('content')
<div class="card" x-data="formHandler('{{ route('units.update', $unit) }}', 'PUT')" x-init="init()">
    <div class="card-header">
        <h2 class="card-title">Edit Unit: {{ $unit->name }}</h2>
        ...
        <form @submit.prevent="submit($event)">
            ...
            <div style="display: flex; gap: 12px; margin-top: 24px;">
                <button type="submit" class="btn btn-primary" :disabled="submitting">
                    <span x-show="!submitting">Update Unit</span>
...
<script>
    function formHandler(url, method) {
        return {
            form: {
                name: '{{ old('name', $unit->name) }}',
                status: '{{ old('status', $unit->status) }}',
            },
            errors: {},
            errorMessage: '',
            successMessage: '',
            ...
            async submit() {
                ...
                try {
                    ...
                    if (response.ok) {
                        window.location.href = data.redirect || '{{ route("units.index") }}';
                        return;
                    }
