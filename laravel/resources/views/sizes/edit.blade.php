...
@section('content')
<div class="card" x-data="formHandler('{{ route('sizes.update', $size) }}', 'PUT')" x-init="init()">
    <div class="card-header">
        <h2 class="card-title">Edit Size: {{ $size->name }}</h2>
        ...
        <form @submit.prevent="submit($event)">
            ...
            <div style="display: flex; gap: 12px; margin-top: 24px;">
                <button type="submit" class="btn btn-primary" :disabled="submitting">
                    <span x-show="!submitting">Update Size</span>
...
<script>
    function formHandler(url, method) {
        return {
            form: {
                name: '{{ old('name', $size->name) }}',
                status: '{{ old('status', $size->status) }}',
            },
            errors: {},
            errorMessage: '',
            successMessage: '',
