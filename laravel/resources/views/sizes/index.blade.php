    ...
    <div class="card">
        <div class="card-header">
            ...
            <div style="margin-left: auto; display: flex; gap: 8px;">
                <a href="{{ route('sizes.export') }}" class="btn btn-secondary">
        ...
        <div style="padding: 16px;">
            <form method="GET" action="{{ route('sizes.index') }}" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
                ...
                @if(request()->hasAny(['search', 'status']))
                    <a href="{{ route('sizes.index') }}" class="btn btn-ghost">Clear</a>
                @endif
            </form>
        </div>
            ...
            <tbody>
                @forelse($sizes as $size)
                    <tr id="row-{{ $size->id }}">
                        ...
                        <td>
                            <span class="badge {{ $size->status === 'active' ? 'badge-green' : 'badge-orange' }}">
                                {{ ucfirst($size->status) }}
                            </span>
                        </td>
                        <td>
                            <div class="actions">
                                <a href="{{ route('sizes.edit', $size) }}" class="btn btn-ghost btn-sm">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                    Edit
                                </a>
                                <button type="button" class="btn btn-ghost btn-sm" style="color: var(--error);" @click="confirmDelete({{ $size->id }}, '{{ route('sizes.destroy', $size) }}', '{{ e($size->name) }}')">
                ...
                @empty
                    <tr>
                        <td colspan="3" class="empty-state">No sizes found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="showModal" x-cloak :class="{ 'show': showModal }" class="modal-overlay" @click.self="cancelDelete()">
        <div class="modal" @click.stop>
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                ...
                <div>
                    <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Delete Size</h3>
                </div>
            </div>
    ...
    <div x-show="showImportModal" x-cloak :class="{ 'show': showImportModal }" class="modal-overlay" @click.self="closeImportModal()">
        <div class="modal" @click.stop>
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                ...
                <div>
                    <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Import Sizes</h3>
