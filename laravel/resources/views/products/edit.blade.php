...
@section('content')
<div class="card" x-data="formHandler('{{ route('products.update', $product) }}', 'PUT')" x-init="init()">
        ...
        <form @submit.prevent="submit($event)">
                ...
                <div class="form-group">
                    ...
                    <select id="fk_brand_id" x-model="form.fk_brand_id">
                        ...
                        @foreach($brands as $brand)
                            <option value="{{ $brand->id }}" {{ $product->fk_brand_id == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <textarea id="description" x-model="form.description" rows="4">{{ old('description', $product->description) }}</textarea>
    ...
    function formHandler(url, method) {
        return {
            form: {
                ...
                fk_brand_id: '{{ old('fk_brand_id', $product->fk_brand_id) }}',
                fk_category_id: '{{ old('fk_category_id', $product->fk_category_id) }}',
                fk_supplier_id: '{{ old('fk_supplier_id', $product->fk_supplier_id) }}',
                fk_unit_id: '{{ old('fk_unit_id', $product->fk_unit_id) }}',
