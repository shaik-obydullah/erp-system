            ...
            <tbody>
                @forelse($boms as $bom)
                    <tr id="row-{{ $bom->id }}">
                        ...
                        <td>
                            <div class="actions">
                                <a href="{{ route('bill-of-materials.edit', $bom) }}" class="btn btn-ghost btn-sm">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                    Edit
                                </a>
                                <button type="button" class="btn btn-ghost btn-sm" style="color: var(--error);" @click="confirmDelete({{ $bom->id }}, '{{ route('bill-of-materials.destroy', $bom) }}', '{{ e($bom->name) }}')">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="empty-state">No bill of materials found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="showModal" x-cloak :class="{ 'show': showModal }" class="modal-overlay" @click.self="cancelDelete()">
        <div class="modal" @click.stop>
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                ...
                <div>
                    <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Delete BOM</h3>
                </div>
            </div>
