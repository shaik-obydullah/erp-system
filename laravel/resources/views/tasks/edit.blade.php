...
@section('content')
<div class="card" x-data="formHandler('{{ route('tasks.update', $task) }}', 'PUT')" x-init="init()">
        ...
        <form @submit.prevent="submit($event)">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="fk_employee_id" x-model="form.fk_employee_id" required>
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ $task->fk_employee_id == $employee->id ? 'selected' : '' }}>{{ $employee->first_name }} {{ $employee->last_name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="task_name" x-model="form.task_name" value="{{ old('task_name', $task->task_name) }}" required autofocus>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <textarea id="description" x-model="form.description" rows="4">{{ old('description', $task->description) }}</textarea>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" id="start_date" x-model="form.start_date" value="{{ old('start_date', $task->start_date) }}">
                    <span class="form-error" x-show="errors.start_date" x-text="errors.start_date"></span>
                </div>
                <div class="form-group">
                    <label for="due_date">Due Date</label>
                    <input type="date" id="due_date" x-model="form.due_date" value="{{ old('due_date', $task->due_date) }}">
...
<script>
    function formHandler(url, method) {
        return {
            form: {
                fk_employee_id: '{{ old('fk_employee_id', $task->fk_employee_id) }}',
                task_name: '{{ old('task_name', $task->task_name) }}',
                description: '{{ old('description', $task->description) }}',
                status: '{{ old('status', $task->status) }}',
                start_date: '{{ old('start_date', $task->start_date) }}',
                due_date: '{{ old('due_date', $task->due_date) }}',
            },
            errors: {},
            errorMessage: '',
            successMessage: '',
