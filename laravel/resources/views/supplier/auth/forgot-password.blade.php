...
@section('content')
    <div x-data="forgotPasswordForm()" x-init="init()">
        ...
        <form method="POST" action="{{ route('supplier.password.email') }}" @submit.prevent="submit()">
            @csrf
        ...
        function forgotPasswordForm() {
            return {
                ...
                async submit() {
                    ...
                    try {
                        const response = await fetch('{{ route('supplier.password.email') }}', {
                            method: 'POST',
                            headers: {
                        ...
                        if (response.ok) {
                            window.location.href = data.redirect || '/supplier/forgot-password';
                            return;
                        }
