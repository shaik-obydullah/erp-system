...
@section('content')
    <div x-data="loginForm()" x-init="init()">
        ...
        <p class="login-subtitle">Use your Supplier Account</p>
        ...
        <form method="POST" action="{{ route('supplier.login') }}" @submit.prevent="submit()">
            @csrf
        ...
        function loginForm() {
            return {
                ...
                async submit() {
                    ...
                    try {
                        const response = await fetch('{{ route('supplier.login') }}', {
                            method: 'POST',
                            headers: {
