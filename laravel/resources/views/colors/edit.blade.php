...
@section('content')
<div class="card" x-data="formHandler('{{ route('colors.update', $color) }}', 'PUT')" x-init="init()">
    <div class="card-header">
        <h2 class="card-title">Edit Color: {{ $color->name }}</h2>
        ...
        <form @submit.prevent="submit($event)">
            ...
            <div style="display: flex; gap: 12px; margin-top: 24px;">
                <button type="submit" class="btn btn-primary" :disabled="submitting">
                    <span x-show="!submitting">Update Color</span>
...
<script>
    function formHandler(url, method) {
        return {
            form: {
                name: '{{ old('name', $color->name) }}',
                status: '{{ old('status', $color->status) }}',
            },
            errors: {},
            errorMessage: '',
            successMessage: '',
            ...
            async submit() {
                ...
                try {
                    ...
                    if (response.ok) {
                        window.location.href = data.redirect || '{{ route("colors.index") }}';
                        return;
                    }
