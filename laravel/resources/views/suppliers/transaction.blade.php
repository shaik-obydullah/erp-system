...
<div class="card" x-data="transactionFilters()"
     data-customer-id="{{ request('supplier_id') }}"
     data-date-from="{{ request('date_from') }}"
     data-date-to="{{ request('date_to') }}"
     data-clear-url="{{ route('suppliers.transaction.index') }}">
    <div class="card-header">
        <h2 class="card-title">Supplier Transaction History</h2>
        <div style="display: flex; gap: 8px;">
            <a href="{{ route('suppliers.transaction.export', request()->query()) }}" class="btn btn-secondary">
    ...
    <div style="padding: 16px;">
        <form method="GET" action="{{ route('suppliers.transaction.index') }}" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
            <select name="supplier_id" class="form-input" style="max-width: 250px;" x-model="customerId">
                    ...
                    <td>
                        <div style="display: flex; align-items: center; gap: 10px;">
                            <div style="width: 32px; height: 32px; background: var(--primary); color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 500; flex-shrink: 0;">
                                {{ $txn->supplier ? substr($txn->supplier->name, 0, 1) : '?' }}
                            </div>
                            <strong>{{ $txn->supplier->name ?? 'N/A' }}</strong>
                        </div>
                    </td>
...
<div x-show="showImportModal" x-cloak :class="{ 'show': showImportModal }" class="modal-overlay" @click.self="closeImportModal()">
    <div class="modal" @click.stop>
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
            ...
            <div>
                <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Import Supplier Transactions</h3>
                <p style="margin: 2px 0 0; font-size: 13px; color: var(--text-secondary);">Upload a CSV file with columns: Supplier, Description, In Amount, Out Amount, Payable, Receivable</p>
            </div>
        </div>
        <form action="{{ route('suppliers.transaction.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
