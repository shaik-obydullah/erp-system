...
<div style="margin-bottom: 24px;">
    <div style="display: flex; gap: 4px; border-bottom: 1px solid var(--border); padding: 0 16px;">
        ...
        <a href="{{ route('reports.income') }}" class="nav-item" style="border-radius: 8px 8px 0 0; margin-bottom: -1px; background: var(--bg-white); color: var(--primary); border-bottom: 2px solid var(--primary);">
...
<div class="card" style="margin-bottom: 24px;">
    <div class="card-body">
        <form method="GET" action="{{ route('reports.income') }}" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
            ...
            @if(request()->hasAny(['start_date', 'end_date']))
                <a href="{{ route('reports.income') }}" class="btn btn-ghost" style="height: 42px;">Clear</a>
            @endif
        </form>
    </div>
</div>
...
<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 28px;">
    <div class="stat-card">
        ...
        <div class="stat-info">
            ...
            <span class="stat-value">{{ $currencySymbol }}{{ number_format($totalIncome, 2) }}</span>
        </div>
    </div>
</div>
        ...
        <tbody>
            @forelse($incomes as $income)
                <tr>
                    ...
                    <td>{{ $currencySymbol }}{{ number_format($income->amount, 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="4" class="empty-state">No income records found for the selected period.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var months = {!! $chartLabelsJson !!};
        var totals = {!! $monthlyIncomeJson !!};
        ...
        new Chart(ctx.getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    borderWidth: 2,
                    borderRadius: 6,
                    hoverBackgroundColor: 'rgba(52, 168, 83, 0.3)',
                    fill: true,
                    tension: 0.4,
                    pointRadius: 4,
