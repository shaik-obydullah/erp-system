    ...
    <div class="card">
        ...
        <div style="padding: 16px;">
            <form method="GET" action="{{ route('customers.index') }}" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
                <input type="text" name="search" value="{{ request('search') }}" class="form-input" placeholder="Search by name, email, or phone..." style="flex: 1; min-width: 180px;">
                ...
                <div style="margin-left: auto; display: flex; gap: 8px;">
                    <a href="{{ route('customers.export') }}" class="btn btn-secondary">
            ...
            <tbody>
                @forelse($customers as $customer)
                    <tr id="row-{{ $customer->id }}">
                        ...
                        <td>{{ $currencySymbol }}{{ number_format($customer->balance, 2) }}</td>
                        <td>
                            <span class="badge {{ $customer->status === 'active' ? 'badge-green' : 'badge-orange' }}">
                        ...
                        <td>
                            <div class="actions">
                                ...
                                <button type="button" class="btn btn-ghost btn-sm" style="color: var(--error);" @click="confirmDelete({{ $customer->id }}, '{{ route('customers.destroy', $customer) }}', '{{ e($customer->name) }}')">
    ...
    <div x-show="showModal" x-cloak :class="{ 'show': showModal }" class="modal-overlay" @click.self="cancelDelete()">
        <div class="modal" @click.stop>
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                ...
                <div>
                    <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Delete Customer</h3>
                </div>
            </div>
    ...
    <div x-show="showImportModal" x-cloak :class="{ 'show': showImportModal }" class="modal-overlay" @click.self="closeImportModal()">
        <div class="modal" @click.stop>
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                ...
                <div>
                    <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Import Customers</h3>
                    <p style="margin: 2px 0 0; font-size: 13px; color: var(--text-secondary);">Upload a CSV file with columns: Name, Email, Phone, Address, Balance, Status</p>
                </div>
            </div>
