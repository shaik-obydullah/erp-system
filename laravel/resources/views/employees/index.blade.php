...
<div x-data="deleteHandler()" x-init="init()">
    <div class="card">
        ...
        <div style="padding: 16px;">
            <form method="GET" action="{{ route('employees.index') }}" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
                <input type="text" name="search" value="{{ request('search') }}" class="form-input" placeholder="Search by name or email..." style="flex: 1; min-width: 200px;">
            ...
            <tbody>
                @forelse($employees as $employee)
                    <tr id="row-{{ $employee->id }}">
                        ...
                        <td>
                            <span class="badge {{ $employee->status === 'active' ? 'badge-green' : ($employee->status === 'inactive' ? 'badge-orange' : 'badge-red') }}">
                        ...
                        <td>
                            <div class="actions">
                                ...
                                <button type="button" class="btn btn-ghost btn-sm" style="color: var(--error);" @click="confirmDelete({{ $employee->id }}, '{{ route('employees.destroy', $employee) }}', '{{ e($employee->first_name . " " . $employee->last_name) }}')">
    ...
    <div x-show="showModal" x-cloak :class="{ 'show': showModal }" class="modal-overlay" @click.self="cancelDelete()">
        <div class="modal" @click.stop>
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                ...
                <div>
                    <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Delete Employee</h3>
                </div>
            </div>
